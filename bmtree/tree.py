...
class Node:
    ...
    def fill_out_tables(self, min_points, max_points, weights): # fill out the table    
        ...
        for i in range(len(min_points)):
            ...
            queries.append(Query(query_list, weights[i]))
            self.query_volumn += queries[i].area * weights[i]
            ...
            for i in range(dim):
                ...
                for rise_index in range(row_len):
                    ...
                    for drop_index in range(column_len):
                        ...
                        self.table[rise_dim][rise_index][drop_index] += rise_res * drop_res * query.weight
            break


    def read_lookup_tables(self, min_points, max_points, actions, dimension_choose, weights):
        self.fill_out_tables(min_points, max_points, weights)
        x_reading_map = {}
        y_reading_map = {}
    ...
    def cal_local_cost(self, min_points, max_points, actions, weights):
        ...
        if self.chosen_bit is None:
            ...
            for max_point, min_point, weight in zip(max_points, min_points, weights):
            ...
            # check if we need to split the query
                if partition_length > 0:
                    ...
                    area = (max_point[0] - min_point[0]) * (max_point[1] - min_point[1])
                    area_left = (left_max_points[0] - left_min_points[0]) * (left_max_points[1] - left_min_points[1])
                    area_right = (right_max_points[0] - right_min_points[0]) * (right_max_points[1] - right_min_points[1])
                    left_weights.append(weight * area_left / area)
                    right_weights.append(weight * area_right / area)
                else:
                    if is_in_left:
                        left_min_points.append(min_point)
            ...
            if self.left_children is None or self.left_children.chosen_bit is None:
                if self.child is not None:
                    if self.child.chosen_bit is None:
                        left_volume, left_res, _ = self.read_lookup_tables(left_min_points, left_max_points, actions, dimension_choose_new, left_weights)
                    else:
                        left_volume, left_res, _ = self.child.cal_local_cost(left_min_points, left_max_points, actions, left_weights)
                else:
                    left_volume, left_res, _ = self.read_lookup_tables(left_min_points, left_max_points, actions, dimension_choose_new, left_weights)
            else:
                left_volume, left_res, _ = self.left_children.cal_local_cost(left_min_points, left_max_points, actions, left_weights)

        if len(right_min_points) > 0:
            if self.right_children is None or self.right_children.chosen_bit is None:
                if self.child is not None:
                    if self.child.chosen_bit is None:
                        right_volume, right_res, _  = self.read_lookup_tables(right_min_points, right_max_points, actions, dimension_choose_new, right_weights)
                    else:
                        right_volume, right_res, _  = self.child.cal_local_cost(right_min_points, right_max_points, actions, right_weights)
                else:
                    right_volume, right_res, _  = self.read_lookup_tables(right_min_points, right_max_points, actions, dimension_choose_new, right_weights)
            else:
                right_volume, right_res, _  = self.right_children.cal_local_cost(right_min_points, right_max_points, actions, right_weights)
...
class Tree:
    ...
    # 
    def output_all_queries_local(self, queries, is_weight):
        min_points = []
        max_points = []
        weights = []
        for query in queries:
            ...
            max_points.append(data_point_transfer_max)
            min_points.append(data_point_transfer_min)
            weights.append(query.weight if is_weight else 1.0)
            # pprint.pprint(data_point_transfer_min)
            # pprint.pprint(data_point_transfer_max)
        # recursively read the table

        volume, res, _ = self.root_node.cal_local_cost(min_points, max_points, [], weights)
